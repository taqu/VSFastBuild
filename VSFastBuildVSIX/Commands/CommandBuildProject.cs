#define VSFASTBUILD_USE_PRECOMPILEDHEADER
using Blake2Fast;
using EnvDTE;
using EnvDTE80;
...
namespace VSFastBuildVSIX
{
    ...
    internal sealed class CommandBuildProject : BaseCommand<CommandBuildProject>
    {
        ...
        public static bool SupportedProject(EnvDTE.Project project)
        {
            return ProjectTypes.WindowsCPlusPlus == project.Kind || ProjectTypes.WindowsCSharp == project.Kind;
        }
        ...
        protected override async Task ExecuteAsync(OleMenuCmdEventArgs e)
        {
            ...
            foreach (SelectedItem item in selectedItems)
            {
                ...
                {
                    if (!SupportedProject(item.Project))
                    {
                        continue;
                    }
        ...
        public static async Task BuildProjectsAsync(VSFastBuildVSIXPackage package, List<EnvDTE.Project> targets, bool fromSolution = false, bool generateOnly = false)
        {
            ...
            List<string> originalTargets = new List<string>(targets.Count);
            foreach (EnvDTE.Project target in targets)
            {
                originalTargets.Add(target.UniqueName);
            }
            ...
            try
            {
                ...
                if (!VSFastBuildVSIXPackage.Options.GenOnly && !generateOnly)
                {
                    ...
                    if (fromSolution)
                    {
                        Result result = results[results.Count - 1];
                        {
                            await Log.OutputBuildLineAsync($"--- VSFastBuild begin running {result.bffName_}---");
                            await RunProcessAsync(result, package, fbuildPath, fbuldArgs);
                            await Log.OutputBuildLineAsync($"--- VSFastBuild end running {result.bffName_}---");
                        }
                    }
                    else
                    {
                        ...
                        {
                            if (!originalTargets.Contains(result.project_.name_))
                            {
                                continue;
                            }
                            await Log.OutputBuildLineAsync($"--- VSFastBuild begin running {result.bffName_}---");
                ...
                await Log.OutputBuildLineAsync($"--- VSFastBuild end ---");
            }
            catch (Exception ex)
            {
        ...
        public static async Task RunProcessAsync(Result result, VSFastBuildVSIXPackage package, string fbuildPath, string fbuildArgs)
        {
...

            {
                try
                {
                    string targetName = string.IsNullOrEmpty(result.project_.intDir_) ? "all" : result.project_.name_;
                    string arguments = $"-config \"{result.bffPath_}\" {targetName} {fbuildArgs}";
                    {
        ...
        public struct CustomBuildItem
        {
            ...
            public bool verifyInputAndOutput_;
        }
        ...
        public class FBCompileItem
        {
            ...
            public string OutputPath => outputPath_;
            ...
            private List<string> includeDirectories_;
            ...
            public FBCompileItem(ItemType type, string outputPath, string inputFile, string options, string outputExtension)
            {
                type_ = type;
            ...
            public bool AddIfMatches(ItemType type, string outputPath, string inputFile, string options, string outputExtension)
            {
                if (type_ == ItemType.Custom)
                {
                    return false;
                }
                if (type_ == type && outputPath_ == outputPath && options_ == options && outputExtension_ == outputExtension)
                {
        ...
        public struct PrecompiledHeaderInfo
        {
            ...
            public string compilerOptions_ = string.Empty;
        }
        ...
        public static List<VSFastProject> GetDependencies(VSFastBuildVSIXPackage package, List<EnvDTE.Project> projects)
        {
            ...
            for (int i = 0; i < projects.Count; ++i)
            {
                ...
                string targetName = projects[i].Name;//projects[i].Name.Replace('-', '_');
        ...
        public static async Task<Result> BuildForProjectAsync(VSFastBuildVSIXPackage package, VSFastProject vsFastProject)
        {
            ...
            {
                string lastbuildstate = $"PlatformToolSet={PlatformToolSet}:VCToolArchitecture={VCToolArchitecture}:VCToolsVersion={VCToolsVersion}:TargetPlatformVersion={vsFastProject.buildProject_.GetProperty("TargetPlatformVersion").EvaluatedValue}:";
                lastbuildstate = $"{lastbuildstate}\r\n{result.project_.configuration_}|{result.project_.platform_}|{result.project_.projectDir_.TrimEnd('\\', '/') + "\\"}|\r\n";
                string tlogDir = System.IO.Path.Combine(vsFastProject.intDir_, vsFastProject.project_.Name + ".tlog");
                string lastbuildstateFile = System.IO.Path.Combine(tlogDir, $"{vsFastProject.project_.Name}.lastbuildstate");
                if (!System.IO.Directory.Exists(tlogDir))
                {
                    System.IO.Directory.CreateDirectory(tlogDir);
                }
                if (!System.IO.File.Exists(lastbuildstateFile))
                {
                    using (StreamWriter writer = new StreamWriter(lastbuildstateFile, false, new System.Text.UTF8Encoding(false)))
                    {
                        writer.Write(lastbuildstate);
                    }
                }
            }
            ...
            if (0 < buildContext.targets_.Count)
            {
                ...
                stringBuilder.AppendLine($"  .ExecInput = '{cleanname}'");
                ...
                //cleanBuilder.AppendLine($"DEL /S /Q \"{vsFastProject.intDir_}*.pdb\"");
                try
                {
...
#if VSFASTBUILD_USE_PRECOMPILEDHEADER
        private static void GetPrecompiledHeader(BuildContext buildContext, VSFastProject project, Microsoft.Build.Evaluation.ProjectItem item)
        {
            if (item.Metadata.Where(x => x.Name == "PrecompiledHeader" && x.EvaluatedValue == "Create").Any())
            {
                project.precompiledHeaderInfo_.PCHInputFile_ = item.EvaluatedInclude.Trim();
                if (!project.precompiledHeaderInfo_.PCHInputFile_.Contains('/') && !project.precompiledHeaderInfo_.PCHInputFile_.Contains('\\'))
                {
                    project.precompiledHeaderInfo_.PCHInputFile_ = System.IO.Path.GetFullPath(System.IO.Path.Combine(project.rootDir_, project.precompiledHeaderInfo_.PCHInputFile_));
                }
                project.precompiledHeaderInfo_.PCHOutputFile_ = item.GetMetadataValue("PrecompiledHeaderOutputFile").Trim().Replace("/", "\\").Replace("\\\\", "\\");
                ...
                string[] propertiesToSkip = new string[] { "ObjectFileName", "AssemblerListingLocation", "ProgramDataBaseFileName", "ModuleOutputFile", "XMLDocumentationFileName", "DiagnosticsFormat" };
                string pchCompilerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref project.compilerPDB_, project.rootDir_, item.Metadata);
#if false
...
#endif
                pchCompilerOptions = pchCompilerOptions.Replace("  ", " ").Replace("\\\\", "\\").Replace("/TP", string.Empty).Replace("/TC", string.Empty).Replace("/D ", "/D");
                if (!project.precompiledHeaderInfo_.PCHInputFile_.EndsWith(".c"))
                {
                    pchCompilerOptions += " /TP ";
                }
                project.precompiledHeaderInfo_.PCHOptions_ = $"\"%1\" /Fo\"%3\" /FS /Fd\"$CompilerPDB$\" {pchCompilerOptions}";
                project.precompiledHeaderInfo_.compilerOptions_ = $"/FS /Fd\"$CompilerPDB$\" {pchCompilerOptions}";
            }
        }
#endif

        private static string GenerateTaskCommandLine(ToolTask task, string[] propertiesToSkip, ref string compilerPDB, string rootDir, IEnumerable<ProjectMetadata> metaDataList)
        {
            ...
            foreach (ProjectMetadata metaData in metaDataList)
            {
                ...
                if (matchingProps.Any() && !string.IsNullOrEmpty(metaData.EvaluatedValue))
                {
                    ...
                    {
                        if ("AdditionalDependencies" == metaData.Name)
                        {
                            string[] dependencies = evaluatedValue.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
                            for (int i = 0; i < dependencies.Length; ++i)
                            {
                                string dependency = dependencies[i].Trim();
                                if (dependency.Contains('\\') || dependency.Contains('/'))
                                {
                                    if (!System.IO.Path.IsPathRooted(dependency))
                                    {
                                        dependencies[i] = System.IO.Path.GetFullPath(System.IO.Path.Combine(rootDir, dependency));
                                    }
                                }
                            }
                            propInfo.SetValue(task, Convert.ChangeType(dependencies, propInfo.PropertyType));
                        }
                        else
                        {
                            propInfo.SetValue(task, Convert.ChangeType(evaluatedValue.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries), propInfo.PropertyType));
                        }
                    }
                    else
                    {
...
#endif
#if !VSFASTBUILD_USE_PRECOMPILEDHEADER
                        else if("PrecompiledHeader" == metaData.Name){
                            evaluatedValue = "NotUsing";
                        }
#endif
        ...
        private static CustomBuildItem CreateCustomBuildCommandline(ICollection<ProjectMetadata> metadataList)
        {
            ...
            customBuildItem.verifyInputAndOutput_ = false;
            foreach (ProjectMetadata metaData in metadataList)
            {
                switch (metaData.Name)
                {
                    ...
                    case "Outputs":
                        ...
                        {
                            customBuildItem.output_ = metaData.EvaluatedValue.Trim().Trim(';');
                        }
                        break;
                    case "Command":
                        customBuildItem.command_ = metaData.EvaluatedValue.Trim();
                        break;
                    case "LinkObjects":
                    ...
                    case "TreatOutputAsContent":
                        ...
                        break;
                    case "VerifyInputsAndOutputsExist":
                        customBuildItem.verifyInputAndOutput_ = "true" == metaData.EvaluatedValue;
                        break;
                }
            }
        ...
        public static void addStringList(StringBuilder stringBuilder, List<string> list, string prefix)
        {
            ...
            {
                stringBuilder.AppendLine($"{prefix}'{list[i]}',");
            }
        }
        ...
        private static string GetOutput(Microsoft.Build.Evaluation.ProjectItem item, VSFastProject project)
        {
            string outputPath = item.GetMetadataValue("ObjectFileName");
            if (null == outputPath)
            {
                return project.intDir_;
            }
            ...
            if (System.IO.Path.HasExtension(outputPath))
            {
                ...
                //    Log.OutputDebugLine($"{metadata.Name}={metadata.EvaluatedValue}");
                //}
                //Log.OutputDebugLine($"{outputPath}");
                outputPath = System.IO.Path.GetDirectoryName(outputPath);
            }
            outputPath = System.IO.Path.Combine(project.rootDir_, outputPath);
            return outputPath;
        }
        ...
        private static void GatherItems(BuildContext buildContext, VSFastProject project)
        {
            ...
            foreach (Microsoft.Build.Evaluation.ProjectItem item in compileItems)
            {
                ...
                GetPrecompiledHeader(buildContext, project, item);
            ...
            { // Resource objects
                ...
                foreach (Microsoft.Build.Evaluation.ProjectItem item in resourceCompileItems)
                {
                    ...
                    string[] propertiesToSkip = new string[] { "ResourceOutputFileName", "DesigntimePreprocessorDefinitions", "ProgramDataBaseFileName", "ModuleOutputFile", "XMLDocumentationFileName", "DiagnosticsFormat" };
                    string resourceCompilerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref dummyPDB, project.rootDir_, item.Metadata);
                    ...
                    string outputPath = GetOutput(item, project);
                    IEnumerable<FBCompileItem> matchingNodes = project.CompileItems.Where(el => el.AddIfMatches(ItemType.Resource, outputPath, evaluatedInclude, formattedCompilerOptions, "res"));
                    if (!matchingNodes.Any())
                    {
                        project.CompileItems.Add(new FBCompileItem(ItemType.Resource, outputPath, evaluatedInclude, formattedCompilerOptions, "res"));
            ...
            { // Compile items
                ...
                foreach (Microsoft.Build.Evaluation.ProjectItem item in compileItems)
                {
                    ...
                    ToolTask task = (ToolTask)Activator.CreateInstance(buildContext.CppTaskAssembly_.GetType("Microsoft.Build.CPPTasks.CL"));
                    string tempCompilerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref project.compilerPDB_, project.rootDir_, item.Metadata);
                    ...
                    optionBuilder = optionBuilder.Replace("\\\\", "\\").Replace("/D ", "/D").Replace("/TP", string.Empty).Replace("/TC", string.Empty);
                    ...
                    string evaluatedInclude = System.IO.Path.GetFullPath(System.IO.Path.Combine(project.rootDir_, item.EvaluatedInclude)).Replace("/", "\\").Replace("\\\\", "\\");
                    string outputPath = GetOutput(item, project);
                    IEnumerable<FBCompileItem> matchingNodes = project.CompileItems.Where(el => el.AddIfMatches(ItemType.CXX, outputPath, evaluatedInclude, formattedCompilerOptions, "obj"));
                    if (!matchingNodes.Any())
                    {
            ...
            // CUDA Compile
            {
                ...
                foreach (Microsoft.Build.Evaluation.ProjectItem item in cudaCompileItems)
                {
                    ...
                    string evaluatedInclude = System.IO.Path.GetFullPath(System.IO.Path.Combine(project.rootDir_, item.EvaluatedInclude)).Replace("/", "\\").Replace("\\\\", "\\");
                    string outputPath = GetOutput(item, project);
                    IEnumerable<FBCompileItem> matchingNodes = project.CompileItems.Where(el => el.AddIfMatches(ItemType.CUDA, outputPath, evaluatedInclude, tempCompilerOptions, "obj"));
                    if (!matchingNodes.Any())
                    {
                        project.CompileItems.Add(new FBCompileItem(ItemType.CUDA, outputPath, evaluatedInclude, tempCompilerOptions, "obj"));
            ...
            // MASM Compile
            {
                ...
                foreach (Microsoft.Build.Evaluation.ProjectItem item in MASMCompileItems)
                {
                    ...
                    string evaluatedInclude = System.IO.Path.GetFullPath(System.IO.Path.Combine(project.rootDir_, item.EvaluatedInclude)).Replace("/", "\\").Replace("\\\\", "\\");
                    string outputPath = GetOutput(item, project);
                    IEnumerable<FBCompileItem> matchingNodes = project.CompileItems.Where(el => el.AddIfMatches(ItemType.MASM, outputPath, evaluatedInclude, tempCompilerOptions, "obj"));
                    if (!matchingNodes.Any())
                    {
                        project.CompileItems.Add(new FBCompileItem(ItemType.MASM, outputPath, evaluatedInclude, tempCompilerOptions, "obj"));
            ...
            // FX Compile
            {
                ...
                foreach (Microsoft.Build.Evaluation.ProjectItem item in FXCompileItems)
                {
                    ...
                    string dummyPDB = string.Empty;
                    string tempCompilerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref dummyPDB, project.rootDir_, item.Metadata);
                    StringBuilder optionBuilder = buildContext.optionBuilder_.Clear();
                    optionBuilder.Append(tempCompilerOptions);
                    optionBuilder.Append(" \"%1\" /Fo\"%2\" ");
                    ...
                    string outputPath = GetOutput(item, project);
                    IEnumerable<FBCompileItem> matchingNodes = project.CompileItems.Where(el => el.AddIfMatches(ItemType.HLSL, outputPath, evaluatedInclude, formattedCompilerOptions, "cso"));
                    if (!matchingNodes.Any())
                    {
                        project.CompileItems.Add(new FBCompileItem(ItemType.HLSL, outputPath, evaluatedInclude, formattedCompilerOptions, "cso"));
        ...
        private static List<string> GetDependentLibraries(List<string> objTargets, VSFastProject project)
        {
                    ...
                    case "DynamicLibrary":
                        {
                            ...
                            if (null != metadata)
                            {
                                ...
                                {
                                    if (!System.IO.Path.IsPathRooted(importLibrary))
                                    {
                                        importLibrary = System.IO.Path.Combine(dependency.rootDir_, importLibrary);
                                    }
                                    if (!libraries.Contains(importLibrary))
                                    {
                                        libraries.Add(importLibrary);
                                    }
                                }
                            }
                        }
                    ...
                    case "StaticLibrary":
                        {
                            ...
                            if (null != metadata)
                            {
                                ...
                                if (!string.IsNullOrEmpty(outputFile))
                                {
                                    ...
                                    {
                                        outputFile = System.IO.Path.Combine(dependency.rootDir_, outputFile);
                                    }
                                    if (!libraries.Contains(outputFile))
                                    {
                                        libraries.Add(outputFile);
                                    }
                                }
                            }
                        }
            ...
            {
                VCProject vcProject = project.project_.Object as VCProject;
                VCReferences vsReferences = vcProject.References as VCReferences;
                References references = vcProject.References as References;
                foreach (VSLangProj.Reference reference in vcProject.References as References)
                {
                    string path = reference.Path;
                    if (path.EndsWith(".lib") || path.EndsWith(".a") || path.EndsWith(".obj") || path.EndsWith(".o"))
                    {
        ...
        private static Tuple<string, string> GetCommandArguments(string commandLine)
        {
            int commandEnd = -1;
            if (0 < commandLine.Length && commandLine[0] == '"')
            {
            ...
            if (commandEnd < 0)
            {
                return new Tuple<string, string>(commandLine, string.Empty);
            }
            string command = commandLine.AsSpan(0, commandEnd).ToString().Trim();
            string arguments = commandEnd < commandLine.Length ? commandLine.AsSpan(commandEnd, commandLine.Length - commandEnd).ToString().Trim() : string.Empty;
            return new Tuple<string, string>(command, arguments);
        }

        private static void OutputCustomBuildItem(StringBuilder stringBuilder, CustomBuildItem customBuildItem, string name, string indent, VSFastProject project)
        {
            if (0 < customBuildItem.command_.Count((c) => { return '\n' == c || '\r' == c; }))
            {
                string batchPath = System.IO.Path.Combine(project.intDir_, $"fbuild_{name}.bat");
                try
                {
                    System.IO.File.WriteAllText(batchPath, customBuildItem.command_);
                    stringBuilder.AppendLine("    .ExecExecutable = 'C:\\Windows\\System32\\cmd.exe'");
                    stringBuilder.AppendLine($"    .ExecArguments = '/C {batchPath}'");
                }
                catch { }
            }
            else
            {
                Tuple<string, string> commandArguments = GetCommandArguments(customBuildItem.command_);
                if (System.IO.Path.IsPathRooted(commandArguments.Item1))
                {
                    stringBuilder.AppendLine($"    .ExecExecutable = '\"{commandArguments.Item1}\"'");
                    stringBuilder.AppendLine($"    .ExecArguments = '{commandArguments.Item2}'");
                }
                else
                {
                    stringBuilder.AppendLine("    .ExecExecutable = 'C:\\Windows\\System32\\cmd.exe'");
                    stringBuilder.AppendLine($"    .ExecArguments = '/C {commandArguments.Item1} {commandArguments.Item2}'");
                }
            }
        }

        private static void PrintDummy(StringBuilder stringBuilder, VSFastProject project, string lastTargetName, string indent)
        {
            try
            {
                if (!System.IO.Directory.Exists(project.intDir_))
                {
                    System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(project.intDir_));
                }
            }
            catch
            {
            ...
            string dummy = System.IO.Path.Combine(project.intDir_, $"{project.targetName_}_dummy.out");
            string dummyName = $"{project.targetName_}_dummy";
            stringBuilder.AppendLine($"{indent}Exec('{dummyName}')");
            stringBuilder.AppendLine($"{indent}{{");
            stringBuilder.AppendLine($"{indent}  .ExecExecutable = 'C:\\Windows\\System32\\cmd.exe'");
            stringBuilder.AppendLine($"{indent}  .ExecArguments = '/C type nul > {dummy}'");
            stringBuilder.AppendLine($"{indent}  .ExecOutput = '{dummy}'");
            stringBuilder.AppendLine($"{indent}}}");

            stringBuilder.AppendLine($"  Alias('{lastTargetName}')");
            stringBuilder.AppendLine($"{indent}{{");
            stringBuilder.AppendLine($"{indent}  .Targets =");
            stringBuilder.AppendLine($"{indent}  {{");
            stringBuilder.AppendLine($"{indent}    '{dummyName}'");
            stringBuilder.AppendLine($"{indent}  }}");
            stringBuilder.AppendLine($"{indent}}}");
        }

        private static void PrintPreBuild(StringBuilder stringBuilder, BuildContext buildContext, VSFastProject project, List<string> dependencies)
        {
            ICollection<Microsoft.Build.Evaluation.ProjectItem> buildEvents = project.buildProject_.GetItems("PreBuildEvent");
            int count = 0;
            foreach (Microsoft.Build.Evaluation.ProjectItem projectItem in buildEvents)
            {
                ProjectMetadata commandMetadata = projectItem.GetMetadata("Command");
                if (null == commandMetadata)
                {
                    continue;
                }
                string command = commandMetadata.EvaluatedValue;
                ...
                string preBuildBatchFile = System.IO.Path.Combine(project.rootDir_, $"fbuild_{name}.bat");
                ...
                stringBuilder.AppendLine("    .ExecUseStdOutAsOutput = false");
                stringBuilder.AppendLine("    .ExecAlways = true");
                stringBuilder.AppendLine($"    .Environment = .{buildContext.envName_}");
                stringBuilder.AppendLine("  }");
        ...
        private static void PrintPostBuild(StringBuilder stringBuilder, BuildContext buildContext, VSFastProject project, string lastTargetName)
        {
            ICollection<Microsoft.Build.Evaluation.ProjectItem> buildEvents = project.buildProject_.GetItems("PostBuildEvent");
            int count = 0;
            ...
            foreach (Microsoft.Build.Evaluation.ProjectItem projectItem in buildEvents)
            {
                ...
                string postBuildBatchFile = System.IO.Path.Combine(project.rootDir_, $"fbuild_{name}.bat");
        ...
        private static void AddProject(BuildContext buildContext, VSFastProject project)
        {
            ...
            if (0 < prebuildCount || project.ExistsFlags[(int)ItemType.Custom])
            {
                ...
                // PreBuild
                PrintPreBuild(stringBuilder, buildContext, project, dependencies);
                ...
                if (project.ExistsFlags[(int)ItemType.Custom])
                {
                    ...
                    {
                        if (false == item.CustomBuildItem.verifyInputAndOutput_)
                        {
                            continue;
                        }
                        ...
                        OutputCustomBuildItem(stringBuilder, item.CustomBuildItem, name, "    ", project);
                        ...
                        if (!string.IsNullOrEmpty(item.CustomBuildItem.output_)
                            && (
                            item.CustomBuildItem.output_.EndsWith(".obj")
                            || item.CustomBuildItem.output_.EndsWith(".o")
                            || item.CustomBuildItem.output_.EndsWith(".lib")
                            || item.CustomBuildItem.output_.EndsWith(".a")))
                        {
                            string path = item.CustomBuildItem.output_;
                            if (!System.IO.Path.IsPathRooted(path))
                            {
                                path = System.IO.Path.GetFullPath(path);
                            }
                            if (!objTargets.Contains(path))
                            {
                                objTargets.Add(path);
                            }
                        }
            ...
            if (project.precompiledHeaderInfo_.IsValid())
            {
                ...
                stringBuilder.AppendLine("  {");
                stringBuilder.AppendLine($"    .Compiler = .{buildContext.compilerName_}");
                stringBuilder.AppendLine($"    .CompilerOptions = '{project.precompiledHeaderInfo_.compilerOptions_}'");
                stringBuilder.AppendLine($"    .CompilerOutputPath = '{System.IO.Path.GetDirectoryName(project.precompiledHeaderInfo_.PCHOutputFile_)}'");
            ...
            // Resource objects
            {
                ...
                foreach (FBCompileItem item in project.CompileItems.Where(item => item.Type == ItemType.Resource))
                {
                    ...
                    stringBuilder.AppendLine($"    .CompilerOutputPath = '{item.OutputPath}'");
            ...
            string fxctargets_deps = string.Empty;
            {
                ...
                foreach (FBCompileItem item in project.CompileItems.Where(item => item.Type == ItemType.HLSL))
                {
                    ...
                    for (int j = 0; j < item.InputFiles.Count; ++j)
                    {
                        ...
                        string targetPath = System.IO.Path.Combine(project.intDir_, System.IO.Path.GetFileNameWithoutExtension(item.InputFiles[j]));
                        buildContext.targets_.Add($"{targetPath}.{item.OutputExtension}");
                    }
                    stringBuilder.AppendLine("    }");
                ...
                case BuildType.Application:
                    {
                        ...
                        if (0 < objTargets.Count)
                        {
                            ...
                            string[] propertiesToSkip = new string[] { "OutputFile", "ProfileGuidedDatabase", "ProgramDatabaseFile", "XMLDocumentationFileName", "DiagnosticsFormat", "LinkTimeCodeGenerationObjectFile", "IncrementalLinkDatabaseFile" };
                            string linkerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref project.linkerPDB_, project.rootDir_, linkDefinitions.Metadata);
                        ...
                        {
                            if (!string.IsNullOrEmpty(dependenciesName) || !string.IsNullOrEmpty(fxctargets_deps))
                            {
                                stringBuilder.AppendLine($"  Alias('{lastTargetName}')");
                                stringBuilder.AppendLine("  {");
                                stringBuilder.AppendLine("    .Targets =");
                            ...
                            {
                                PrintDummy(stringBuilder, project, lastTargetName, "  ");
                            }
                        }
                    }
                    break;
                case BuildType.StaticLib:
                    {
                        ...
                        {
                            stringBuilder.AppendLine($"    .CompilerOptions = '\"%1\" /Fo\"%2\" /c /TP /Fd\"$CompilerPDB$\" /FS'");
                        }
                        else
                        {
                        ...
                        string[] propertiesToSkip = new string[] { "OutputFile", "ProgramDatabaseFile", "XMLDocumentationFileName", "DiagnosticsFormat" };
                        string linkerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref project.linkerPDB_, project.rootDir_, libDefinitions.Metadata);
                        StringBuilder optionBuilder = buildContext.optionBuilder_.Clear();
                ...
                case BuildType.DynamicLib:
                    {
                        ...
                        ToolTask task = (ToolTask)Activator.CreateInstance(buildContext.CppTaskAssembly_.GetType("Microsoft.Build.CPPTasks.Link"));
                        string[] propertiesToSkip = new string[] { "OutputFile", "ProfileGuidedDatabase", "XMLDocumentationFileName", "DiagnosticsFormat", "LinkTimeCodeGenerationObjectFile", "IncrementalLinkDatabaseFile" };
                        string linkerOptions = GenerateTaskCommandLine(task, propertiesToSkip, ref project.linkerPDB_, project.rootDir_, linkDefinitions.Metadata);
                        linkerOptions = linkerOptions.Replace("'", "^'");
                        StringBuilder optionBuilder = buildContext.optionBuilder_.Clear();
                        optionBuilder.Append("\"%1[0]\" /OUT:\"%2\" /PDB:\"$LinkerPDB$\"");
            ...
            // PostBuild
            PrintPostBuild(stringBuilder, buildContext, project, lastTargetName);

            // objects
            if (0 < objTargets.Count)
            {
                string objectsName = $"{project.targetName_}_objects";
                stringBuilder.AppendLine($"  Alias('{objectsName}')");
                stringBuilder.AppendLine("  {");
                stringBuilder.AppendLine("    .Targets =");
                stringBuilder.AppendLine("    {");
